...
final class P31Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P31Monitor> {

	P31Monitor_Set(){
		this.size = 0;
		this.elements = new P31Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P31Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P31Monitor monitorfinalMonitor = monitor;
...
class P31Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P31Monitor ret = (P31Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {3, 3, 5, 3, 4, 5};;
	static final int Prop_1_transition_q[] = {0, 1, 5, 3, 4, 5};;
	static final int Prop_1_transition_r[] = {4, 4, 5, 2, 4, 5};;
	static final int Prop_1_transition_s[] = {1, 1, 5, 3, 4, 5};;
	static final int Prop_1_transition_t[] = {0, 4, 5, 3, 4, 5};;
	static final int Prop_1_transition_z[] = {0, 1, 5, 3, 4, 5};;
...
public final class P31RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P31MapManager;
	static {
		P31MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P31MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P31_RVMLock = new ReentrantLock();
	static final Condition P31_RVMLock_cond = P31_RVMLock.newCondition();

	private static boolean P31_activated = false;

	// Declarations for Indexing Trees
	private static Object P31_v_Map_cachekey_v;
	private static P31Monitor P31_v_Map_cachevalue;
	private static final MapOfMonitor<P31Monitor> P31_v_Map = new MapOfMonitor<P31Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P31_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P31_activated = true;
		while (!P31_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P31Monitor> matchedLastMap = null;
		P31Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P31_v_Map_cachekey_v) ) {
			matchedEntry = P31_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P31Monitor> itmdMap = P31_v_Map;
				matchedLastMap = itmdMap;
				P31Monitor node_v = P31_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P31Monitor created = new P31Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P31Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P31_v_Map_cachekey_v = v;
			P31_v_Map_cachevalue = matchedEntry;
		}
