...
final class P4Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P4Monitor> {

	P4Monitor_Set(){
		this.size = 0;
		this.elements = new P4Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P4Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P4Monitor monitorfinalMonitor = monitor;
...
class P4Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P4Monitor ret = (P4Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_r[] = {0, 0, 3, 3};;
...
public final class P4RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P4MapManager;
	static {
		P4MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P4MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P4_RVMLock = new ReentrantLock();
	static final Condition P4_RVMLock_cond = P4_RVMLock.newCondition();

	private static boolean P4_activated = false;

	// Declarations for Indexing Trees
	private static Object P4_v_Map_cachekey_v;
	private static P4Monitor P4_v_Map_cachevalue;
	private static final MapOfMonitor<P4Monitor> P4_v_Map = new MapOfMonitor<P4Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P4_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P4_activated = true;
		while (!P4_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P4Monitor> matchedLastMap = null;
		P4Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P4_v_Map_cachekey_v) ) {
			matchedEntry = P4_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P4Monitor> itmdMap = P4_v_Map;
				matchedLastMap = itmdMap;
				P4Monitor node_v = P4_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P4Monitor created = new P4Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P4Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P4_v_Map_cachekey_v = v;
			P4_v_Map_cachevalue = matchedEntry;
		}
