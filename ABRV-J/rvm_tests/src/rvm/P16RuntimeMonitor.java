...
final class P16Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P16Monitor> {

	P16Monitor_Set(){
		this.size = 0;
		this.elements = new P16Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P16Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P16Monitor monitorfinalMonitor = monitor;
...
class P16Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P16Monitor ret = (P16Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 1, 4, 3, 4};;
	static final int Prop_1_transition_q[] = {1, 1, 4, 3, 4};;
	static final int Prop_1_transition_r[] = {3, 2, 4, 3, 4};;
	static final int Prop_1_transition_s[] = {1, 1, 4, 3, 4};;
	static final int Prop_1_transition_t[] = {1, 1, 4, 3, 4};;
	static final int Prop_1_transition_z[] = {1, 1, 4, 3, 4};;
...
public final class P16RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P16MapManager;
	static {
		P16MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P16MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P16_RVMLock = new ReentrantLock();
	static final Condition P16_RVMLock_cond = P16_RVMLock.newCondition();

	private static boolean P16_activated = false;

	// Declarations for Indexing Trees
	private static Object P16_v_Map_cachekey_v;
	private static P16Monitor P16_v_Map_cachevalue;
	private static final MapOfMonitor<P16Monitor> P16_v_Map = new MapOfMonitor<P16Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P16_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P16_activated = true;
		while (!P16_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P16Monitor> matchedLastMap = null;
		P16Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P16_v_Map_cachekey_v) ) {
			matchedEntry = P16_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P16Monitor> itmdMap = P16_v_Map;
				matchedLastMap = itmdMap;
				P16Monitor node_v = P16_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P16Monitor created = new P16Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P16Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P16_v_Map_cachekey_v = v;
			P16_v_Map_cachevalue = matchedEntry;
		}
