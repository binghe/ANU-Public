...
final class P26Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P26Monitor> {

	P26Monitor_Set(){
		this.size = 0;
		this.elements = new P26Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P26Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P26Monitor monitorfinalMonitor = monitor;
...
class P26Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P26Monitor ret = (P26Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {1, 1, 4, 3, 4};;
	static final int Prop_1_transition_q[] = {0, 1, 4, 3, 4};;
	static final int Prop_1_transition_r[] = {3, 2, 4, 3, 4};;
	static final int Prop_1_transition_s[] = {0, 0, 4, 3, 4};;
	static final int Prop_1_transition_t[] = {0, 1, 4, 3, 4};;
	static final int Prop_1_transition_z[] = {0, 1, 4, 3, 4};;
...
public final class P26RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P26MapManager;
	static {
		P26MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P26MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P26_RVMLock = new ReentrantLock();
	static final Condition P26_RVMLock_cond = P26_RVMLock.newCondition();

	private static boolean P26_activated = false;

	// Declarations for Indexing Trees
	private static Object P26_v_Map_cachekey_v;
	private static P26Monitor P26_v_Map_cachevalue;
	private static final MapOfMonitor<P26Monitor> P26_v_Map = new MapOfMonitor<P26Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P26_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P26_activated = true;
		while (!P26_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P26Monitor> matchedLastMap = null;
		P26Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P26_v_Map_cachekey_v) ) {
			matchedEntry = P26_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P26Monitor> itmdMap = P26_v_Map;
				matchedLastMap = itmdMap;
				P26Monitor node_v = P26_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P26Monitor created = new P26Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P26Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P26_v_Map_cachekey_v = v;
			P26_v_Map_cachevalue = matchedEntry;
		}
