...
final class P8Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P8Monitor> {

	P8Monitor_Set(){
		this.size = 0;
		this.elements = new P8Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P8Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P8Monitor monitorfinalMonitor = monitor;
...
class P8Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P8Monitor ret = (P8Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 0, 3, 3};;
	static final int Prop_1_transition_q[] = {1, 1, 3, 3};;
	static final int Prop_1_transition_r[] = {0, 2, 3, 3};;
...
public final class P8RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P8MapManager;
	static {
		P8MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P8MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P8_RVMLock = new ReentrantLock();
	static final Condition P8_RVMLock_cond = P8_RVMLock.newCondition();

	private static boolean P8_activated = false;

	// Declarations for Indexing Trees
	private static Object P8_v_Map_cachekey_v;
	private static P8Monitor P8_v_Map_cachevalue;
	private static final MapOfMonitor<P8Monitor> P8_v_Map = new MapOfMonitor<P8Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P8_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P8_activated = true;
		while (!P8_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P8Monitor> matchedLastMap = null;
		P8Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P8_v_Map_cachekey_v) ) {
			matchedEntry = P8_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P8Monitor> itmdMap = P8_v_Map;
				matchedLastMap = itmdMap;
				P8Monitor node_v = P8_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P8Monitor created = new P8Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P8Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P8_v_Map_cachekey_v = v;
			P8_v_Map_cachevalue = matchedEntry;
		}
