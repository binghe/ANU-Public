...
final class P23Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P23Monitor> {

	P23Monitor_Set(){
		this.size = 0;
		this.elements = new P23Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P23Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P23Monitor monitorfinalMonitor = monitor;
...
class P23Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P23Monitor ret = (P23Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 1, 4, 1, 4};;
	static final int Prop_1_transition_q[] = {3, 1, 4, 3, 4};;
	static final int Prop_1_transition_r[] = {0, 2, 4, 0, 4};;
	static final int Prop_1_transition_s[] = {0, 1, 4, 0, 4};;
...
public final class P23RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P23MapManager;
	static {
		P23MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P23MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P23_RVMLock = new ReentrantLock();
	static final Condition P23_RVMLock_cond = P23_RVMLock.newCondition();

	private static boolean P23_activated = false;

	// Declarations for Indexing Trees
	private static Object P23_v_Map_cachekey_v;
	private static P23Monitor P23_v_Map_cachevalue;
	private static final MapOfMonitor<P23Monitor> P23_v_Map = new MapOfMonitor<P23Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P23_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P23_activated = true;
		while (!P23_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P23Monitor> matchedLastMap = null;
		P23Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P23_v_Map_cachekey_v) ) {
			matchedEntry = P23_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P23Monitor> itmdMap = P23_v_Map;
				matchedLastMap = itmdMap;
				P23Monitor node_v = P23_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P23Monitor created = new P23Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P23Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P23_v_Map_cachekey_v = v;
			P23_v_Map_cachevalue = matchedEntry;
		}
