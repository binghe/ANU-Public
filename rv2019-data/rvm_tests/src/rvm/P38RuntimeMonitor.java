...
final class P38Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P38Monitor> {

	P38Monitor_Set(){
		this.size = 0;
		this.elements = new P38Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P38Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P38Monitor monitorfinalMonitor = monitor;
...
class P38Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P38Monitor ret = (P38Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 1, 5, 3, 0, 5};;
	static final int Prop_1_transition_q[] = {4, 1, 5, 3, 4, 5};;
	static final int Prop_1_transition_r[] = {0, 0, 5, 2, 0, 5};;
	static final int Prop_1_transition_s[] = {0, 1, 5, 3, 1, 5};;
	static final int Prop_1_transition_t[] = {0, 3, 5, 3, 4, 5};;
...
public final class P38RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P38MapManager;
	static {
		P38MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P38MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P38_RVMLock = new ReentrantLock();
	static final Condition P38_RVMLock_cond = P38_RVMLock.newCondition();

	private static boolean P38_activated = false;

	// Declarations for Indexing Trees
	private static Object P38_v_Map_cachekey_v;
	private static P38Monitor P38_v_Map_cachevalue;
	private static final MapOfMonitor<P38Monitor> P38_v_Map = new MapOfMonitor<P38Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P38_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P38_activated = true;
		while (!P38_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P38Monitor> matchedLastMap = null;
		P38Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P38_v_Map_cachekey_v) ) {
			matchedEntry = P38_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P38Monitor> itmdMap = P38_v_Map;
				matchedLastMap = itmdMap;
				P38Monitor node_v = P38_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P38Monitor created = new P38Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P38Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P38_v_Map_cachekey_v = v;
			P38_v_Map_cachevalue = matchedEntry;
		}
