...
final class P36Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P36Monitor> {

	P36Monitor_Set(){
		this.size = 0;
		this.elements = new P36Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P36Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P36Monitor monitorfinalMonitor = monitor;
...
class P36Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P36Monitor ret = (P36Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {2, 5, 2, 3, 4, 5};;
	static final int Prop_1_transition_q[] = {0, 5, 2, 3, 4, 5};;
	static final int Prop_1_transition_r[] = {2, 5, 2, 2, 1, 5};;
	static final int Prop_1_transition_s[] = {3, 5, 2, 3, 4, 5};;
	static final int Prop_1_transition_t[] = {0, 5, 2, 4, 4, 5};;
...
public final class P36RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P36MapManager;
	static {
		P36MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P36MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P36_RVMLock = new ReentrantLock();
	static final Condition P36_RVMLock_cond = P36_RVMLock.newCondition();

	private static boolean P36_activated = false;

	// Declarations for Indexing Trees
	private static Object P36_v_Map_cachekey_v;
	private static P36Monitor P36_v_Map_cachevalue;
	private static final MapOfMonitor<P36Monitor> P36_v_Map = new MapOfMonitor<P36Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P36_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P36_activated = true;
		while (!P36_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P36Monitor> matchedLastMap = null;
		P36Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P36_v_Map_cachekey_v) ) {
			matchedEntry = P36_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P36Monitor> itmdMap = P36_v_Map;
				matchedLastMap = itmdMap;
				P36Monitor node_v = P36_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P36Monitor created = new P36Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P36Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P36_v_Map_cachekey_v = v;
			P36_v_Map_cachevalue = matchedEntry;
		}
