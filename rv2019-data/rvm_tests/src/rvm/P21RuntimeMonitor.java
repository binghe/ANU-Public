...
final class P21Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P21Monitor> {

	P21Monitor_Set(){
		this.size = 0;
		this.elements = new P21Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P21Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P21Monitor monitorfinalMonitor = monitor;
...
class P21Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P21Monitor ret = (P21Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {2, 1, 2, 4, 4};;
...
public final class P21RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P21MapManager;
	static {
		P21MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P21MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P21_RVMLock = new ReentrantLock();
	static final Condition P21_RVMLock_cond = P21_RVMLock.newCondition();

	private static boolean P21_activated = false;

	// Declarations for Indexing Trees
	private static Object P21_v_Map_cachekey_v;
	private static P21Monitor P21_v_Map_cachevalue;
	private static final MapOfMonitor<P21Monitor> P21_v_Map = new MapOfMonitor<P21Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P21_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P21_activated = true;
		while (!P21_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P21Monitor> matchedLastMap = null;
		P21Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P21_v_Map_cachekey_v) ) {
			matchedEntry = P21_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P21Monitor> itmdMap = P21_v_Map;
				matchedLastMap = itmdMap;
				P21Monitor node_v = P21_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P21Monitor created = new P21Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P21Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P21_v_Map_cachekey_v = v;
			P21_v_Map_cachevalue = matchedEntry;
		}
