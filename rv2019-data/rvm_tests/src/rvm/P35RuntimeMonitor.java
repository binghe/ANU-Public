...
final class P35Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P35Monitor> {

	P35Monitor_Set(){
		this.size = 0;
		this.elements = new P35Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P35Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P35Monitor monitorfinalMonitor = monitor;
...
class P35Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P35Monitor ret = (P35Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_r[] = {0, 4, 2, 3, 4};;
	static final int Prop_1_transition_s[] = {2, 4, 2, 3, 4};;
	static final int Prop_1_transition_t[] = {0, 4, 1, 3, 4};;
...
public final class P35RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P35MapManager;
	static {
		P35MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P35MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P35_RVMLock = new ReentrantLock();
	static final Condition P35_RVMLock_cond = P35_RVMLock.newCondition();

	private static boolean P35_activated = false;

	// Declarations for Indexing Trees
	private static Object P35_v_Map_cachekey_v;
	private static P35Monitor P35_v_Map_cachevalue;
	private static final MapOfMonitor<P35Monitor> P35_v_Map = new MapOfMonitor<P35Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P35_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P35_activated = true;
		while (!P35_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P35Monitor> matchedLastMap = null;
		P35Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P35_v_Map_cachekey_v) ) {
			matchedEntry = P35_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P35Monitor> itmdMap = P35_v_Map;
				matchedLastMap = itmdMap;
				P35Monitor node_v = P35_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P35Monitor created = new P35Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P35Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P35_v_Map_cachekey_v = v;
			P35_v_Map_cachevalue = matchedEntry;
		}
