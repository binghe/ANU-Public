...
final class P51Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P51Monitor> {

	P51Monitor_Set(){
		this.size = 0;
		this.elements = new P51Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P51Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P51Monitor monitorfinalMonitor = monitor;
...
class P51Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P51Monitor ret = (P51Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {1, 1, 2, 5, 1, 5};;
	static final int Prop_1_transition_q[] = {0, 1, 2, 5, 4, 5};;
	static final int Prop_1_transition_r[] = {2, 3, 2, 5, 3, 5};;
	static final int Prop_1_transition_s[] = {0, 4, 2, 5, 4, 5};;
	static final int Prop_1_transition_t[] = {0, 1, 2, 5, 0, 5};;
	static final int Prop_1_transition_z[] = {0, 1, 2, 5, 1, 5};;
...
public final class P51RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P51MapManager;
	static {
		P51MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P51MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P51_RVMLock = new ReentrantLock();
	static final Condition P51_RVMLock_cond = P51_RVMLock.newCondition();

	private static boolean P51_activated = false;

	// Declarations for Indexing Trees
	private static Object P51_v_Map_cachekey_v;
	private static P51Monitor P51_v_Map_cachevalue;
	private static final MapOfMonitor<P51Monitor> P51_v_Map = new MapOfMonitor<P51Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P51_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P51_activated = true;
		while (!P51_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P51Monitor> matchedLastMap = null;
		P51Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P51_v_Map_cachekey_v) ) {
			matchedEntry = P51_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P51Monitor> itmdMap = P51_v_Map;
				matchedLastMap = itmdMap;
				P51Monitor node_v = P51_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P51Monitor created = new P51Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P51Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P51_v_Map_cachekey_v = v;
			P51_v_Map_cachevalue = matchedEntry;
		}
