...
final class P15Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P15Monitor> {

	P15Monitor_Set(){
		this.size = 0;
		this.elements = new P15Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P15Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P15Monitor monitorfinalMonitor = monitor;
...
class P15Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P15Monitor ret = (P15Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_r[] = {1, 2, 2};;
	static final int Prop_1_transition_s[] = {1, 2, 2};;
	static final int Prop_1_transition_t[] = {1, 2, 2};;
	static final int Prop_1_transition_z[] = {1, 2, 2};;
...
public final class P15RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P15MapManager;
	static {
		P15MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P15MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P15_RVMLock = new ReentrantLock();
	static final Condition P15_RVMLock_cond = P15_RVMLock.newCondition();

	private static boolean P15_activated = false;

	// Declarations for Indexing Trees
	private static Object P15_v_Map_cachekey_v;
	private static P15Monitor P15_v_Map_cachevalue;
	private static final MapOfMonitor<P15Monitor> P15_v_Map = new MapOfMonitor<P15Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P15_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P15_activated = true;
		while (!P15_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P15Monitor> matchedLastMap = null;
		P15Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P15_v_Map_cachekey_v) ) {
			matchedEntry = P15_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P15Monitor> itmdMap = P15_v_Map;
				matchedLastMap = itmdMap;
				P15Monitor node_v = P15_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P15Monitor created = new P15Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P15Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P15_v_Map_cachekey_v = v;
			P15_v_Map_cachevalue = matchedEntry;
		}
