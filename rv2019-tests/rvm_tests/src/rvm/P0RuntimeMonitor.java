...
final class P0Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P0Monitor> {

	P0Monitor_Set(){
		this.size = 0;
		this.elements = new P0Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P0Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P0Monitor monitorfinalMonitor = monitor;
...
class P0Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P0Monitor ret = (P0Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {1, 2, 2};;
	static final int Prop_1_transition_q[] = {0, 2, 2};;
...
public final class P0RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P0MapManager;
	static {
		P0MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P0MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P0_RVMLock = new ReentrantLock();
	static final Condition P0_RVMLock_cond = P0_RVMLock.newCondition();

	private static boolean P0_activated = false;

	// Declarations for Indexing Trees
	private static Object P0_v_Map_cachekey_v;
	private static P0Monitor P0_v_Map_cachevalue;
	private static final MapOfMonitor<P0Monitor> P0_v_Map = new MapOfMonitor<P0Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P0_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P0_activated = true;
		while (!P0_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P0Monitor> matchedLastMap = null;
		P0Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P0_v_Map_cachekey_v) ) {
			matchedEntry = P0_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P0Monitor> itmdMap = P0_v_Map;
				matchedLastMap = itmdMap;
				P0Monitor node_v = P0_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P0Monitor created = new P0Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P0Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P0_v_Map_cachekey_v = v;
			P0_v_Map_cachevalue = matchedEntry;
		}
