...
final class P20Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P20Monitor> {

	P20Monitor_Set(){
		this.size = 0;
		this.elements = new P20Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P20Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P20Monitor monitorfinalMonitor = monitor;
...
class P20Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P20Monitor ret = (P20Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {2, 1, 3, 3};;
	static final int Prop_1_transition_q[] = {0, 1, 3, 3};;
	static final int Prop_1_transition_r[] = {0, 1, 3, 3};;
	static final int Prop_1_transition_s[] = {1, 1, 3, 3};;
...
public final class P20RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P20MapManager;
	static {
		P20MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P20MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P20_RVMLock = new ReentrantLock();
	static final Condition P20_RVMLock_cond = P20_RVMLock.newCondition();

	private static boolean P20_activated = false;

	// Declarations for Indexing Trees
	private static Object P20_v_Map_cachekey_v;
	private static P20Monitor P20_v_Map_cachevalue;
	private static final MapOfMonitor<P20Monitor> P20_v_Map = new MapOfMonitor<P20Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P20_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P20_activated = true;
		while (!P20_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P20Monitor> matchedLastMap = null;
		P20Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P20_v_Map_cachekey_v) ) {
			matchedEntry = P20_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P20Monitor> itmdMap = P20_v_Map;
				matchedLastMap = itmdMap;
				P20Monitor node_v = P20_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P20Monitor created = new P20Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P20Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P20_v_Map_cachekey_v = v;
			P20_v_Map_cachevalue = matchedEntry;
		}
