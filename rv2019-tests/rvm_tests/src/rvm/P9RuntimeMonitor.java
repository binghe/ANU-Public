...
final class P9Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P9Monitor> {

	P9Monitor_Set(){
		this.size = 0;
		this.elements = new P9Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P9Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P9Monitor monitorfinalMonitor = monitor;
...
class P9Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P9Monitor ret = (P9Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 3, 0, 3};;
	static final int Prop_1_transition_q[] = {2, 3, 2, 3};;
	static final int Prop_1_transition_r[] = {0, 3, 1, 3};;
...
public final class P9RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P9MapManager;
	static {
		P9MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P9MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P9_RVMLock = new ReentrantLock();
	static final Condition P9_RVMLock_cond = P9_RVMLock.newCondition();

	private static boolean P9_activated = false;

	// Declarations for Indexing Trees
	private static Object P9_v_Map_cachekey_v;
	private static P9Monitor P9_v_Map_cachevalue;
	private static final MapOfMonitor<P9Monitor> P9_v_Map = new MapOfMonitor<P9Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P9_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P9_activated = true;
		while (!P9_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P9Monitor> matchedLastMap = null;
		P9Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P9_v_Map_cachekey_v) ) {
			matchedEntry = P9_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P9Monitor> itmdMap = P9_v_Map;
				matchedLastMap = itmdMap;
				P9Monitor node_v = P9_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P9Monitor created = new P9Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P9Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P9_v_Map_cachekey_v = v;
			P9_v_Map_cachevalue = matchedEntry;
		}
