...
final class P29Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P29Monitor> {

	P29Monitor_Set(){
		this.size = 0;
		this.elements = new P29Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P29Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P29Monitor monitorfinalMonitor = monitor;
...
class P29Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P29Monitor ret = (P29Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 1, 1, 4, 4};;
	static final int Prop_1_transition_q[] = {2, 1, 2, 4, 4};;
	static final int Prop_1_transition_r[] = {0, 3, 0, 4, 4};;
	static final int Prop_1_transition_s[] = {0, 2, 2, 4, 4};;
...
public final class P29RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P29MapManager;
	static {
		P29MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P29MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P29_RVMLock = new ReentrantLock();
	static final Condition P29_RVMLock_cond = P29_RVMLock.newCondition();

	private static boolean P29_activated = false;

	// Declarations for Indexing Trees
	private static Object P29_v_Map_cachekey_v;
	private static P29Monitor P29_v_Map_cachevalue;
	private static final MapOfMonitor<P29Monitor> P29_v_Map = new MapOfMonitor<P29Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P29_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P29_activated = true;
		while (!P29_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P29Monitor> matchedLastMap = null;
		P29Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P29_v_Map_cachekey_v) ) {
			matchedEntry = P29_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P29Monitor> itmdMap = P29_v_Map;
				matchedLastMap = itmdMap;
				P29Monitor node_v = P29_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P29Monitor created = new P29Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P29Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P29_v_Map_cachekey_v = v;
			P29_v_Map_cachevalue = matchedEntry;
		}
