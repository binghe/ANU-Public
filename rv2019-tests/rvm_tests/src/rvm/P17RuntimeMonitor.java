...
final class P17Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P17Monitor> {

	P17Monitor_Set(){
		this.size = 0;
		this.elements = new P17Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P17Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P17Monitor monitorfinalMonitor = monitor;
...
class P17Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P17Monitor ret = (P17Monitor) super.clone();
			return ret;
		}
...
public final class P17RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P17MapManager;
	static {
		P17MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P17MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P17_RVMLock = new ReentrantLock();
	static final Condition P17_RVMLock_cond = P17_RVMLock.newCondition();

	private static boolean P17_activated = false;

	// Declarations for Indexing Trees
	private static Object P17_v_Map_cachekey_v;
	private static P17Monitor P17_v_Map_cachevalue;
	private static final MapOfMonitor<P17Monitor> P17_v_Map = new MapOfMonitor<P17Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P17_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P17_activated = true;
		while (!P17_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P17Monitor> matchedLastMap = null;
		P17Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P17_v_Map_cachekey_v) ) {
			matchedEntry = P17_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P17Monitor> itmdMap = P17_v_Map;
				matchedLastMap = itmdMap;
				P17Monitor node_v = P17_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P17Monitor created = new P17Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P17Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P17_v_Map_cachekey_v = v;
			P17_v_Map_cachevalue = matchedEntry;
		}
