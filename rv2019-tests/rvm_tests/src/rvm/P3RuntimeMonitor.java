...
final class P3Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P3Monitor> {

	P3Monitor_Set(){
		this.size = 0;
		this.elements = new P3Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P3Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P3Monitor monitorfinalMonitor = monitor;
...
class P3Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P3Monitor ret = (P3Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_s[] = {0, 1, 4, 3, 4};;
...
public final class P3RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P3MapManager;
	static {
		P3MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P3MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P3_RVMLock = new ReentrantLock();
	static final Condition P3_RVMLock_cond = P3_RVMLock.newCondition();

	private static boolean P3_activated = false;

	// Declarations for Indexing Trees
	private static Object P3_v_Map_cachekey_v;
	private static P3Monitor P3_v_Map_cachevalue;
	private static final MapOfMonitor<P3Monitor> P3_v_Map = new MapOfMonitor<P3Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P3_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P3_activated = true;
		while (!P3_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P3Monitor> matchedLastMap = null;
		P3Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P3_v_Map_cachekey_v) ) {
			matchedEntry = P3_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P3Monitor> itmdMap = P3_v_Map;
				matchedLastMap = itmdMap;
				P3Monitor node_v = P3_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P3Monitor created = new P3Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P3Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P3_v_Map_cachekey_v = v;
			P3_v_Map_cachevalue = matchedEntry;
		}
