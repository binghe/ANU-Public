...
final class P34Monitor_Set extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<P34Monitor> {

	P34Monitor_Set(){
		this.size = 0;
		this.elements = new P34Monitor[4];
	}
	final void event_p(Vector v) {
		int numAlive = 0 ;
		for(int i = 0; i < this.size; i++){
			P34Monitor monitor = this.elements[i];
			if(!monitor.isTerminated()){
				elements[numAlive] = monitor;
				numAlive++;

				final P34Monitor monitorfinalMonitor = monitor;
...
class P34Monitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	protected Object clone() {
		try {
			P34Monitor ret = (P34Monitor) super.clone();
			return ret;
		}
	...
	static final int Prop_1_transition_p[] = {0, 4, 1, 1, 4};;
	static final int Prop_1_transition_q[] = {3, 4, 3, 3, 4};;
	static final int Prop_1_transition_r[] = {0, 4, 0, 0, 4};;
	static final int Prop_1_transition_s[] = {0, 4, 2, 2, 4};;
	static final int Prop_1_transition_t[] = {0, 4, 0, 3, 4};;
...
public final class P34RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager P34MapManager;
	static {
		P34MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		P34MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock P34_RVMLock = new ReentrantLock();
	static final Condition P34_RVMLock_cond = P34_RVMLock.newCondition();

	private static boolean P34_activated = false;

	// Declarations for Indexing Trees
	private static Object P34_v_Map_cachekey_v;
	private static P34Monitor P34_v_Map_cachevalue;
	private static final MapOfMonitor<P34Monitor> P34_v_Map = new MapOfMonitor<P34Monitor>(0) ;

	public static int cleanUp() {
		int collected = 0;
		// indexing trees
		collected += P34_v_Map.cleanUpUnnecessaryMappings();
		return collected;
	}
	...
	public static final void pEvent(Vector v) {
		P34_activated = true;
		while (!P34_RVMLock.tryLock()) {
			Thread.yield();
		}

		CachedWeakReference wr_v = null;
		MapOfMonitor<P34Monitor> matchedLastMap = null;
		P34Monitor matchedEntry = null;
		boolean cachehit = false;
		if ((v == P34_v_Map_cachekey_v) ) {
			matchedEntry = P34_v_Map_cachevalue;
			cachehit = true;
		}
		else {
			...
			{
				// FindOrCreateEntry
				MapOfMonitor<P34Monitor> itmdMap = P34_v_Map;
				matchedLastMap = itmdMap;
				P34Monitor node_v = P34_v_Map.getNodeEquivalent(wr_v) ;
				matchedEntry = node_v;
			}
		}
		// D(X) main:1
		if ((matchedEntry == null) ) {
			...
			// D(X) main:4
			P34Monitor created = new P34Monitor() ;
			matchedEntry = created;
			matchedLastMap.putNode(wr_v, created) ;
		}
		// D(X) main:8--9
		final P34Monitor matchedEntryfinalMonitor = matchedEntry;
		...
		if ((cachehit == false) ) {
			P34_v_Map_cachekey_v = v;
			P34_v_Map_cachevalue = matchedEntry;
		}
